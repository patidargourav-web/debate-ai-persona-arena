...
const gradientBg =
  "bg-gradient-to-br from-[#10152b] via-[#191c24] to-[#0e1413]"; // darker base colors
const accentShape =
  "absolute rounded-full opacity-20 blur-2xl pointer-events-none mix-blend-lighten";
  ...
  return (
    <SpotlightCursor>
      ...
      >
        ...
        <div
          className={`${accentShape} w-[440px] h-[440px] bg-primary top-[-120px] left-[-120px] animate-float`}
        ></div>
        <div
          className={`${accentShape} w-[320px] h-[280px] bg-gradient-to-br from-accent to-secondary right-[-100px] top-1/3 animate-float`}
          style={{ animationDelay: "1.2s" }}
        ></div>
        <div
          className={`${accentShape} w-[200px] h-[140px] bg-muted bottom-10 left-1/2 animate-float`}
        ...
        <div className="relative z-10 flex flex-col items-center w-full">
          {/* Header */}
          <SpotlightReveal radius={350} className="w-full">
            <div className="text-center mb-12 animate-slide-up w-full">
              <h1
                className="text-6xl sm:text-7xl font-extrabold gradient-text tracking-tighter drop-shadow-lg mb-4 font-sans"
          ...
          {/* Features - each card lights up */}
          <div className="w-full max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7 mb-20">
            {features.map((feature, idx) => (
              <SpotlightReveal
                key={feature.title}
                radius={200}
                className={`card-feature p-7 pt-10 flex flex-col items-center justify-between text-center bg-card/80 rounded-2xl shadow-xl border border-primary/20 cursor-pointer animate-fade-in`}
